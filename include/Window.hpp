...
/*! @fn Window::Window(HWND window_handler_)
    @brief Create a Window object passing window handler object
    @param window_handler_
*/

/*! @fn HWND Window::getWindowHandler()
    @brief Get Window's handler object
    @return Window's HWND object
*/

/*! @fn void Window::moveWindow(const POINT2& current_mouse_position, const bool& is_first_click)
    @brief Change Window's position depending on current mouse position:
    @det Calls private "updateValues" method in order to update Window's size and position;
    @det If it is a first click, remembers distance between the mouse cursor and the window;
    @det Calculates new Window's position;
    @det Moves the window using "MoveWindow" from Windows.h.
    @param current_mouse_position Current mouse position
    @param is_first_click Is it the first click
*/

/*! @fn void Window::updateValues()
